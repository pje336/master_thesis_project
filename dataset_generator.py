...
def dataset_generator(patient_id, scan_id, m_phases, f_phase, root_path, ct_path_dict):
    ...
    Tensors have shape [len(m_phases),2,z,x,y]
    ...
    :return stacked_tensor: torch.tensor shape [len(m_phases),2,z,x,y] tensor with CT data of all fixed and moving images.
    """

    moving_tensor = torch.tensor(CT_dataset(patient_id, scan_id, m_phases, root_path, ct_path_dict), dtype=torch.float)
    # repeat fixed image data to match the moving image array shape.
    fixed_tensor =  torch.tensor(np.repeat(CT_dataset(patient_id, scan_id, [f_phase], root_path, ct_path_dict), len(m_phases),
                              axis=0), dtype=torch.float)

    stacked_tensor = torch.stack([fixed_tensor,moving_tensor],1)

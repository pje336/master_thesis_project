...
def deform_contour(flow_field_array, scan_key, root_path, z_shape):
    ...
    for roi_index, roi_name in enumerate(roi_names):
        ...
        for i, flow_field in enumerate(flow_fields_upsampled):
            ...
            # slice_viewer([warped_contour[i,roi_index].detach().numpy(),contour_moving,contour_fixed], flow_field=flow_field[0].detach().numpy())
...
voxelmorph_models = ["training_2022_06_30_07_11_34"]
#
...
# evaluation_scan_keys = [['107','05-26-1999-p4-39328', ['50', '0']],['107','05-26-1999-p4-39328', ['0', '50']]]
...
for moving_tensor, fixed_tensor, scan_key in evaluation_set:
    ...
    for i, model in enumerate(model_array_lab):
        start_time = datetime.now()
        flowfield = LabIRN_predict_flow(model, fixed_tensor, moving_tensor)

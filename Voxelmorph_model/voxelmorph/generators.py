...
def volgen(
    ...
    while True:
        ...
        imgs = [Voxelmorph_model.voxelmorph.py.utils.load_volfile(vol_names[i], **load_params) for i in indices]
        ...
        if segs is True:
            ...
            s = [Voxelmorph_model.voxelmorph.py.utils.load_volfile(vol_names[i], **load_params) for i in indices]
            vols.append(np.concatenate(s, axis=0))
        elif isinstance(segs, list):
            # assume segs is a corresponding list of files or preloaded volumes
            s = [Voxelmorph_model.voxelmorph.py.utils.load_volfile(segs[i], **load_params) for i in indices]
            vols.append(np.concatenate(s, axis=0))
...
def semisupervised(vol_names, seg_names, labels, atlas_file=None, downsize=2):
    ...
    if atlas_file:
        trg_vol = Voxelmorph_model.voxelmorph.py.utils.load_volfile(atlas_file, np_var='vol',
                                                                    add_batch_axis=True, add_feat_axis=True)
        trg_seg = Voxelmorph_model.voxelmorph.py.utils.load_volfile(atlas_file, np_var='seg',
                                                                    add_batch_axis=True, add_feat_axis=True)
...
def conditional_template_creation(vol_names, atlas, attributes,
    ...
    while True:
        ...
        vols = [Voxelmorph_model.voxelmorph.py.utils.load_volfile(vol_names[i], **load_params) for i in indices]
...
def surf_semisupervised(
    ...
    if labels is not None:
        atlas_seg = Voxelmorph_model.voxelmorph.py.utils.filter_labels(atlas_seg, labels)
    else:
    ...
    std_to_surf = lambda x, y: Voxelmorph_model.voxelmorph.py.utils.sdt_to_surface_pts(
        x, y,
    ...
    for li, label in enumerate(labels):  # if only one label, get surface points here
        ...
        atlas_label_vols[li] = Voxelmorph_model.voxelmorph.py.utils.clean_seg(atlas_label_vols[li], smooth_seg_std)
        atlas_sdt[li] = Voxelmorph_model.voxelmorph.py.utils.vol_to_sdt(
    ...
    if nb_labels_sample == len(labels):
        nb_surface_pts_sel = Voxelmorph_model.voxelmorph.py.utils.get_surface_pts_per_label(nb_surface_pts, layer_edge_ratios)
        for li, label in enumerate(labels):  # if only one label, get surface points here
    ...
    while True:
        ...
        X = next(gen)
        X_img = X[0]
        X_seg = Voxelmorph_model.voxelmorph.py.utils.filter_labels(X[1], labels)
        ...
        if nb_labels_sample != len(labels):
            ...
            nb_surface_pts_sel = Voxelmorph_model.voxelmorph.py.utils.get_surface_pts_per_label(
        ...
        for li, sli in enumerate(sel_label_idxs):
            ...
            X_label = Voxelmorph_model.voxelmorph.py.utils.clean_seg_batch(X_label, smooth_seg_std)
            X_sdt_k[..., li] = Voxelmorph_model.voxelmorph.py.utils.vol_to_sdt_batch(

...
def dataset_generators(batch_size):
    ...
    training_generator = generate_dataset(training_scan_keys, root_path_data, ct_path_dict, dimensions, shift,
                                          batch_size)
    validation_set = generate_dataset(random_validation_keys, root_path_data, ct_path_dict, dimensions, shift, batch_size = 1)
...
def train_lvl1(model_dir, batch_size, loss_criterium):
    print("Training lvl1...")
    ...
    training_generator, validation_set, number_of_training_samples, number_of_validation_samples = dataset_generators(batch_size)
    ...
    #     loss_training[:, 0:3000] = temp_lossall[:, 0:3000]
    mean_old_loss = -10
    relative_loss = -1
    epoch = 0

    while (epoch < epochs_lvl1_max and (relative_loss > loss_criterium or relative_loss < 0)) or epochs_lvl1_min <= epoch:
        step_training = 0
        step_validation = 0
        ...
        for X, Y, scan_id in training_generator:
            ...
            # sys.stdout.write(
            #     "\r" + 'step_ "{0}" -> training loss "{1:.4f}" - sim_NCC "{2:4f}" - Jdet "{3:.10f}" -smo "{4:.4f} -reg_c "{5:.4f}"'.format(
            #         step_training, loss.item(), loss_multiNCC.item(), loss_Jacobian.item(), loss_regulation.item(),
            #         reg_code[0].item()))
            # sys.stdout.flush()
        ...
        print("Epoch {} of {}".format(epoch + 1, epochs_lvl1_max))
        print(datetime.now(), flush=True)
        modelname = model_dir + '/' + model_name + "stagelvl1_" + str(epoch) + '.pth'
        torch.save(model.state_dict(), modelname)
        np.save(model_dir + '/training_loss' + model_name + "stagelvl1_" + str(epoch) + '.npy', loss_training)
        np.save(model_dir + '/validation_loss' + model_name + "stagelvl1_" + str(epoch) + '.npy', loss_validation)
        epoch += 1
...
def train_lvl2(model_dir, batch_size):
    ...
    while epoch < epochs_lvl2:
        ...
        for X, Y, scan_id in training_generator:
            ...
            # with lr 1e-3 + with bias
            if step_training == freeze_step // batch_size:
        ...
        torch.save(model.state_dict(), modelname)
        np.save(model_dir + '/training_loss' + model_name + "stagelvl2_" + str(epoch) + '.npy', loss_training)
        np.save(model_dir + '/validation_loss' + model_name + "stagelvl2_" + str(epoch) + '.npy', loss_validation)
        epoch += 1
...
def train_lvl3(model_dir, batch_size):
    ...
    while epoch < epochs_lvl3:
        ...
        torch.save(model.state_dict(), modelname)
        np.save(model_dir + '/training_loss' + model_name + "stagelvl3_" + str(epoch) + '.npy', loss_training)
        np.save(model_dir + '/validation_loss' + model_name + "stagelvl3_" + str(epoch) + '.npy', loss_validation)
        epoch += 1
...
train_lvl1(model_dir, batch_size=20, loss_criterium = 0.005)
torch.cuda.empty_cache()

...
class Miccai2021_LDR_conditional_laplacian_unit_disp_add_lvl1(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(80, 256, 256), range_flow=0.4, number_res_blocks = 5, number_of_res_filters = 4):
        super(Miccai2021_LDR_conditional_laplacian_unit_disp_add_lvl1, self).__init__()
        self.in_channel = in_channel
        self.n_classes = n_classes
        self.start_channel = start_channel
        self.number_of_res_filters = number_of_res_filters
        ...
        self.resblock_group_lvl1 = self.resblock_seq(self.start_channel * self.number_of_res_filters, self.start_channel * self.number_of_res_filters, bias_opt=bias_opt)

        # self.up = torch.nn.Upsample(scale_factor=2, mode="trilinear")
        self.up = nn.ConvTranspose3d(self.start_channel * self.number_of_res_filters, self.start_channel * self.number_of_res_filters, 2, stride=2,
...
class Miccai2021_LDR_conditional_laplacian_unit_disp_add_lvl_general(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(80, 256, 256), range_flow=0.4, number_res_blocks = 5, number_of_res_filters = 4, e0_filter_previous_model = 64, model_previous=None):
        ...
        self.up = nn.ConvTranspose3d(self.start_channel * self.number_of_res_filters,
                                     self.start_channel * self.number_of_res_filters, 2, stride=2,
                                     padding=0, output_padding=0, bias=bias_opt)

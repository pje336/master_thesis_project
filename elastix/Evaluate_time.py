...
if calculate_MSE_and_jac:
    results_file_MSE_JAC = root_path_model + "/results_elastix_MSE_JAC.csv"
    ...
    for scan_id in ct_path_dict[str(patient_id)].keys():
        for f_phase in phases:
            ...
            for m_phase in phases:
                ...
                path_prediction = output_folder + '{}_to_{}/elastix.log'.format(m_phase, f_phase)
                with open(path_prediction, "r") as f:
                    ...
                    try:
                        times.append(60*float(min) + float(sec))
                    except:
                        None
                    # print(times)
